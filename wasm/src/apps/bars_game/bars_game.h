...
struct BarsGameConfig {
    ...
    int move_variance = 280;  // variance when generating each future state
                              // (larger = easier to hit 0/2000)
    int num_choices = 2;
};

/**
 * Bars game: state is an int array (e.g. size 4) in [min_val, max_val].
 * Each turn the game offers num_choices future states (each = current + delta +
 * variance). User picks one; new state = that future state (no extra
 * randomness). Game ends when any value hits min_val or max_val.
 */
class BarsGame {
   public:
    ...
    int state_size() const {
        return config_.state_size;
    }
    int num_choices() const {
        return config_.num_choices;
    }
    int min_val() const {
        return config_.min_val;
    }
    int max_val() const {
        return config_.max_val;
    }
