...
function get_latex_term(indeterminate: string, factor: number, degree: number) {
    if (factor == 0) {
        return "";
    }
    if (degree == 0) {
...
export function poly_to_latex_low_to_high(poly: number[]) {
...
export function poly_to_latex_high_to_low(poly: number[]) {
    let indeterminate: string = 'x'
    let terms = []
    for (let degree = 0; degree < poly.length; degree++) {
        let factor = poly[degree]
